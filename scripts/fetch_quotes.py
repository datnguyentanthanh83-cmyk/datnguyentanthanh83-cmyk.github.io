...
def fetch_sjc_gold():
    """SJC 9999 VND/lượng from vang.today (fallback giavang.now)."""
    urls = (
    ...
    for url in urls:
        try:
            data = get_json(url, timeout=12.0, retries=1)
            src = "vang.today" if "vang.today" in url else "giavang.now"
            break
        except Exception as e:
            last_err = e
    if not data or not data.get("success"):
        raise ValueError(f"SJC fetch failed: {last_err}")
    buy = data.get("buy")
    sell = data.get("sell")
    if buy is None or sell is None:
        raise ValueError("SJC buy/sell missing")
    buy = float(buy)
    sell = float(sell)
    mid = round((buy + sell) / 2.0)
    chg_sell = data.get("change_sell")
    chg_buy = data.get("change_buy")
    chg_abs = None
    if chg_sell is not None:
        chg_abs = float(chg_sell)
    elif chg_buy is not None:
        chg_abs = float(chg_buy)
    chg_pct = None
    if chg_abs is not None and mid:
        # approximate mid change pct from sell change vs prior sell approx
        prior_mid = mid - chg_abs
        if prior_mid:
            chg_pct = chg_abs / prior_mid * 100.0
    return {
        "price": mid,
        "buy": buy,
        "sell": sell,
        ...
        "unit": "VND/luong",
        "source": src,
        "name": data.get("name") or "SJC 9999",
        "apiTime": data.get("time"),
        "apiDate": data.get("date"),
    }
...
def main():
    ...
    try:
        sjc = fetch_sjc_gold()
        world = fresh.get("gold") or previous.get("gold") or {}
        if world.get("price") is not None:
            sjc["worldPrice"] = world["price"]
            sjc["worldChgPct"] = world.get("chgPct")
        fresh["goldsjc"] = sjc
    except Exception as e:
        errors.append(f"goldsjc:{e}")
        if "goldsjc" in previous and previous["goldsjc"].get("price") is not None:
            kept = dict(previous["goldsjc"])
            world = fresh.get("gold") or {}
            if world.get("price") is not None:
                kept["worldPrice"] = world["price"]
                kept["worldChgPct"] = world.get("chgPct")
            kept["retained"] = True
            fresh["goldsjc"] = kept
            retained.append("goldsjc")
    ...
    payload = {
        ...
        "provider": "Investing.com (vs prior close) + VCB/SBV USDVND + SJC (vang.today)",
        "quotes": fresh,
        "errors": errors,
        "retained": retained,
    ...
    for k in ("dxy", "ust10y", "eurusd", "usdjpy", "usdvnd", "goldsjc", "gold", "vnindex", "wti", "btc"):
        q = fresh.get(k)
        if q:
            flag = " [retained]" if q.get("retained") else ""
            extra = ""
            if k == "goldsjc":
                extra = f" buy={q.get('buy')} sell={q.get('sell')} world={q.get('worldPrice')}"
            print(f"  {k}: {q.get('price')} chgPct={q.get('chgPct')}{extra}{flag}")
